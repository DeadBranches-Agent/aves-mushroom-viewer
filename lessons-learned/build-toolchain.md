...
7. `./flutterw analyze` with the libre flavor *applied* reports ~65 errors in `plugins/aves_services_google` (Play-flavor plugin without its deps); with the committed play-flavor pubspec it is clean. For a quick signal under an applied flavor, use `.flutter/bin/dart analyze lib test`.
